"""
Stress Performance Test

Purpose:
    Determine the maximum sustainable capacity of the Product
    Intelligence Service under heavy traffic.

Workload:
    - 250 concurrent users
    - Spawn rate: 20 users/sec
    - Duration: 15 minutes
    - Wait time: 1–2 seconds

Endpoint Distribution:
    - GET /api/v1/products                  -> 40%
    - GET /api/v1/products/{id}/profile     -> 45%
    - GET /api/v1/products/top              -> 15%
"""

from random import choice

from locust import HttpUser, task, between

from config import API_KEY
from test_data import (
    API_PRODUCTS,
    API_PRODUCT_PROFILE,
    API_TOP_PRODUCTS,
    DEFAULT_PAGE,
    DEFAULT_PAGE_SIZE,
    PRODUCT_IDS,
    TOP_PRODUCTS_LIMIT,
    TOP_PRODUCTS_METRIC,
)


class StressUser(HttpUser):
    """
    Simulates heavy production traffic to identify
    system bottlenecks and maximum sustainable capacity.
    """

    wait_time = between(1, 2)

    def on_start(self):
        self.client.headers.update(
            {
                "X-API-Key": API_KEY,
                "Accept": "application/json",
            }
        )

    @task(40)
    def list_products(self):
        self.client.get(
            f"{API_PRODUCTS}?page={DEFAULT_PAGE}&page_size={DEFAULT_PAGE_SIZE}",
            name="GET /products",
        )

    @task(45)
    def product_profile(self):
        self.client.get(
            API_PRODUCT_PROFILE.format(
                product_id=choice(PRODUCT_IDS),
            ),
            name="GET /products/{id}/profile",
        )

    @task(15)
    def top_products(self):
        self.client.get(
            f"{API_TOP_PRODUCTS}?metric={TOP_PRODUCTS_METRIC}&limit={TOP_PRODUCTS_LIMIT}",
            name="GET /products/top",
        )
